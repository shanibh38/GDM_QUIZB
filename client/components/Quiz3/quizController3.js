...
angular.module('citiesApp')
  .controller('quizController3', ['$location', '$rootScope', '$scope', 'skob', '$http', function ($location, $rootScope, $scope, skob, $http) {
    self = this;
    self.contToTOM = true;
    $rootScope.inQuiz3 = true;
    ...
    var x = setInterval(function () {
      if (!$rootScope.stopQuiz3) {
        var now = new Date().getTime();
        var distance = curPlusDate - now;
        var minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
        var seconds = Math.floor((distance % (1000 * 60)) / 1000);
        document.getElementById("demo3").innerHTML = makeMeTwoDigits(minutes) + ":" + makeMeTwoDigits(seconds);
        if (distance < 0) {
          clearInterval(x);
          $rootScope.isExpired3 = true;
    ...
    function saveDetails() {
      if ($scope.moves < 60 || $scope.moves > 78)
      ...
      data = {
        ...
        "resetNum": $rootScope.reset3,
        "firstBoxToMove": $rootScope.firstBox3,
        "endTime": document.getElementById("demo3").innerHTML,
        "totalMoves": $rootScope.movesQuiz3,
        "histMoves": $rootScope.histQ3.toString(),
      };

      $http.post(serverUrl + "Users/addQuiz3B", data)
    ...
    self.checkFirstQuiz = function () {
      if (document.getElementById("under_quiz3_first").value != document.getElementById("under_quiz3_sec").value)
    ...
    function checkDetails() {
      if (
        ...
        ($scope.firstB1 + $scope.firstB2 + $scope.firstB3 + $scope.firstB4!= 100) ||
        ($scope.secondB1 + $scope.secondB2 + $scope.secondB3 + $scope.secondB4 != 100)
      )
        return false;
      else
        return true;
    ...
    function checkAgainFirst() {
      if (document.getElementById("under_quiz3_first").value != document.getElementById("under_quiz3_sec").value &&
        document.getElementById("under_quiz3_first").value != "" && document.getElementById("under_quiz3_sec").value != "" &&
        $scope.moves > 0
      )
        return true;
      else
